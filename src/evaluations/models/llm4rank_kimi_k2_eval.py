...
def main():
    ap = argparse.ArgumentParser(description="Pairwise eval with Kimi-K2 chat-judge over HF Router (no llm4ranking).")
    ...
    ap.add_argument("--model", default="moonshotai/Kimi-K2-Instruct-0905:groq")
    ...
    for i, r in enumerate(tqdm(df.itertuples(index=False), total=len(df), desc="scoring")):
        pid = getattr(r, "prompt_id"); a = int(getattr(r, "movieA")); b = int(getattr(r, "movieB"))
        ...
        if args.show_samples and i < args.show_samples:
            print(f"[sample] pid={pid} A={a} B={b}  margin={m:.4f}")
        margins.append(float(m)); y_list.append(float(getattr(r, "y")))
    ...
    print(f"\nKimi-K2 chat-judge — split={args.split} tol={args.tie_tol}")
