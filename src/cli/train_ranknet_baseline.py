...
class LinearRankNet(nn.Module):
    ...
    def forward(self, dz):  # dz: [B,D]
        return dz @ self.w  # margin since dz = zA - zB
...
# --------- NEW: A/B shuffle helper (train-time only) ----------
def _apply_ab_shuffle_batch(X, y, meta, feature_cols, mode="none", prob=0.5):
    """
    Randomly flip A/B for a subset of the batch.
    - Negate ONLY Δz columns (those starting with 'dz_')
    - Flip label sign y
    Modes:
      "none" : no flips
      "all"  : flip with probability `prob` for any row
      "easy" : flip with probability `prob` but only for difficulty == "easy"
    """
    if mode == "none" or prob <= 0:
        return X, y

    B = y.shape[0]
    base_mask = (torch.rand(B, device=y.device) < prob)

    if mode == "easy":
        diffs = meta.get("difficulty", ["UNK"] * B)
        is_easy = torch.tensor([d == "easy" for d in diffs], device=y.device)
        mask = base_mask & is_easy
    else:
        mask = base_mask

    if mask.any():
        dz_idx = [i for i, c in enumerate(feature_cols) if c.startswith("dz_")]
        if dz_idx:
            X_mask = X[mask]
            X_mask[:, dz_idx] = -X_mask[:, dz_idx]
            X[mask] = X_mask
        y[mask] = -y[mask]
    return X, y
# -------------------------------------------------------------
...
def main():
    ...
    # NEW: train-time A/B shuffling flags
    ap.add_argument("--ab_shuffle", choices=["none","all","easy"], default="none",
                    help="Randomly swap A/B (negate Δz, flip y) during TRAIN only.")
    ap.add_argument("--ab_shuffle_prob", type=float, default=0.5,
                    help="Probability to flip a row (within mode).")
    args = ap.parse_args()
    ...
    for ep in range(1, args.epochs+1):
        ...
        for X, y, meta in tr:
            X = X.to(device)
            y = y.to(device)
            # ------- NEW: apply train-time A/B shuffle -------
            X, y = _apply_ab_shuffle_batch(
                X, y, meta, feature_cols,
                mode=args.ab_shuffle, prob=args.ab_shuffle_prob
            )
            # -------------------------------------------------
